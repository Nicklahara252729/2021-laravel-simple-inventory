...
class EloquentGudangKimiaRepositories implements GudangKimiaRepositories
{
    private $barang;
    private $takeOut;
    private $restock;
    ...
    {
        return $this->barang->where('jenis_gudang','gudang kimia')
            ->orderBy('id', 'desc')
            ->get();
    }
    ...
    {
        $cekData = $this->barang->where(['nama' => $req['nama'], 'jenis_gudang' => "gudang kimia"])
            ->count();
